...
use crate::theme::{ThemeOrigin, UiTheme};
...
impl OmarchyColors {
    ...
    /// Maps Omarchy's semantic colours onto TUIForge's UI roles.
    pub fn to_ui_theme(&self, name: &str) -> UiTheme {
        ...
        UiTheme {
            ...
            panel_background: self.background.mix(self.foreground, 0.08),
...
/// Loads the active Omarchy theme as a [`UiTheme`].
pub fn load_theme(paths: &OmarchyPaths) -> Result<UiTheme, OmarchyError> {
...
"##;
    ...
    fn parses_full_colors_toml() {
        ...
        assert_eq!(c.accent, Color::Rgb(0x7a, 0xa2, 0xf7));
        let t = c.to_ui_theme("Tokyo Night");
        assert_eq!(t.origin, ThemeOrigin::Omarchy);
        assert_eq!(t.error, c.palette[1]);
        assert_eq!(t.warning, c.palette[3]);
        assert_eq!(t.success, c.palette[2]);
        assert_eq!(t.muted, c.palette[8]);
        assert_ne!(t.panel_background, t.background);
    ...
    fn partial_theme_uses_fallbacks() {
        ...
        let t = c.to_ui_theme("Minimal");
        // color8 is not RGB, so muted is derived from fg/bg.
        assert!(matches!(t.muted, Color::Rgb(..)));
    }
    ...
    fn load_theme_from_fixture_directory() {
        let dir = std::env::temp_dir().join(format!("tuiforge-omarchy-{}", std::process::id()));
