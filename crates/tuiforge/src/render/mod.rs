...
use tuiforge_core::{CellBuffer, CellContent, CellStyle, Color, Position, Rect};
        ...
        match &cell.content {
            ...
            CellContent::Glyph(g) => {
                ...
                if g.is_wide() && sx + 1 >= area.right() {
                    // The tail would fall outside the area: show a blank
                    // instead of a half-clipped wide glyph.
            ...
            CellContent::WideTail => {
                // Only reset when the head was actually drawn to our left.
                if pos.x > viewport.x {
                    target.reset();
                    target.set_style(to_ratatui_style(cell.style, depth));
                } else {
                    // The head is scrolled out of view: draw a blank.
                    target
                        .set_symbol(" ")
...
mod tests {
    use super::*;
    use tuiforge_core::{Cell, Document, Grapheme, Size, composite};
    ...
    fn doc_with(cells: &[(u16, u16, &str)]) -> Document {
        let mut doc = Document::new(Size::new(6, 3)).unwrap();
        for (x, y, s) in cells {
            doc.put_cell(
                Position::new(*x, *y),
    ...
    fn draws_viewport_with_offset_and_clipping() {
        let doc = doc_with(&[(0, 0, "a"), (5, 2, "z"), (2, 1, "漢")]);
        let cells = composite(&doc);
        ...
        // Row 1 of the document from x=1: ". 漢 <" -> screen row 1: " 漢" + reset tail
        assert_eq!(buf[(1, 1)].symbol(), " ");
        assert_eq!(buf[(2, 1)].symbol(), "漢");
        assert_eq!(buf[(3, 1)].symbol(), " ");
        // Row 2 from x=1..4: all empty -> spaces; z at x=5 is outside the viewport.
        assert_eq!(row_symbols(&buf, 2, 6), "    ".to_owned() + "  ");
        // Outside the area nothing was written (empty buffer cells are spaces too,
        // but check that (0,0) stays untouched by comparing style).
        assert_eq!(buf[(0, 0)], ratatui::buffer::Cell::EMPTY);
    }

    #[test]
    fn wide_glyph_at_right_edge_of_area_is_blanked() {
        let doc = doc_with(&[(2, 0, "漢")]);
        let cells = composite(&doc);
