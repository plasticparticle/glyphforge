...
use crate::render::{draw_cells, to_ratatui_color};
...
pub fn render(app: &App, frame: &mut Frame<'_>, block_area: Rect, canvas_area: Rect) {
    ...
    let paper = Style::default()
        ...
        .bg(to_ratatui_color(app.theme.background, depth));
    ...
    if let Some(cells) = app.cached_composite() {
        ...
        let cells = tuiforge_core::composite(&app.doc);
        draw_cells(&cells, viewport, canvas_area, buf, depth, paper);
    }
...
mod tests {
    ...
    use tuiforge_core::{Position, Size};
    ...
    fn renders_typed_text_inside_the_canvas_border() {
        ...
        assert!(rows[0].contains("TUIForge"));
    }
    ...
    fn viewport_scrolls_to_keep_cursor_visible() {
        ...
        // Canvas area is 78x6 (border). The cursor sits at 151,60 after typing.
        let vp = app.editor.viewport;
        assert_eq!(vp.size, Size::new(78, 6));
        assert_eq!(vp.offset, Position::new(151 - 78 + 1, 60 - 6 + 1));
    ...
    fn panels_render_layer_list_and_keys() {
        ...
        let all = rows.join("\n");
        assert!(all.contains("Background"), "{all}");
