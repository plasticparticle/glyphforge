...
/// specification. Every directory already has the `glyphforge` suffix.
...
pub const APP_DIR_NAME: &str = "glyphforge";
...
impl AppDirs {
    ...
    pub fn log_file(&self) -> PathBuf {
        self.state.join("glyphforge.log")
    }
}
...
mod tests {
    ...
    fn defaults_relative_to_home() {
        ...
        assert_eq!(dirs.config, PathBuf::from("/home/lars/.config/glyphforge"));
        assert_eq!(
            dirs.data,
            PathBuf::from("/home/lars/.local/share/glyphforge")
        );
        assert_eq!(
            dirs.state,
            PathBuf::from("/home/lars/.local/state/glyphforge")
        );
        assert_eq!(dirs.cache, PathBuf::from("/home/lars/.cache/glyphforge"));
        assert_eq!(
            dirs.config_file(),
            PathBuf::from("/home/lars/.config/glyphforge/config.toml")
        );
    }
    ...
    fn explicit_xdg_variables_win() {
        ...
        assert_eq!(dirs.config, PathBuf::from("/etc/xdg-user/glyphforge"));
        assert_eq!(dirs.state, PathBuf::from("/var/state/glyphforge"));
        assert_eq!(
            dirs.data,
            PathBuf::from("/home/lars/.local/share/glyphforge")
        );
